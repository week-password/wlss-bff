...
@pytest.fixture
def access_token_for_account_owner():
    return jwt.encode({"account_id": 1}, key="some-secret-key")


@pytest.fixture
def get_account_wishes_request(access_token_for_account_owner):
    return Predicate(
        method="GET",
        path="/accounts/1/wishes",
        headers={"Authorization": f"Bearer {access_token_for_account_owner}"},
    )
